...
| --- | --- | --- | --- |
| Claude | `.claude/skills`, `.claude/commands`, `.claude/agents`; always/glob rules in `.claude/rules`; model-decision rules as `rule-` skills | Hooks and allow grants in `.claude/settings.json`; local MCP in `.mcp.json` | Pure output/schema checks and current docs. Native permission matching and menu behavior pending. |
| Codex | Standard skills and `wf-` workflow skills in `.agents/skills` | Local MCP in `.codex/config.toml` managed block | Independent TOML semantics; isolated native Codex 0.153.4 project-config loading and skills/list discovery. Invocation/permission behavior pending. |
