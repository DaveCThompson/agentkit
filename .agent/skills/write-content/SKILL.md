...
When the request names an existing writing style, run the read-only
`node .agent/skills/manage-writing-style/scripts/writing-style.mjs resolve <style-id>` from the project
root to validate and load its reviewed contract and active approved examples. A missing, disabled,
ambiguous, draft or invalid style cannot be silently applied. Continue style-independent writing where
useful and state the unavailable context; do not initialize or ingest samples to satisfy a prose request.
If the script/runtime is unavailable, use the management skill's schema to inspect existing data with
the same boundaries or report the unresolved validation. A style profile is project-local data,
...
2. Establish a semantic brief before wording the prose; a small edit needs no separate document.
   Capture the main point, attribution, supporting claims, sequence,
