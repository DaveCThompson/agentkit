...
export const COMPILER_VERSION = 'writing-style-0.2.0';
...
export function writingRoot(projectRoot) {
  return contained(normalizePath(projectRoot), path.join(normalizePath(projectRoot), '.writing'));
}

function styleRoot(projectRoot, styleId) {
  const root = writingRoot(projectRoot);
  return contained(root, path.join(root, 'styles', assertStyleId(styleId)));
}
...
function contained(root, target) {
  ...
  for (const part of relative.split(path.sep)) {
    ...
    if (stat.isSymbolicLink() || (stat.isFile() && stat.nlink > 1)) throw new StyleError('unsafe-path', 'writing data must use private regular files and directories');
    if (!stat.isDirectory() && !stat.isFile()) throw new StyleError('unsafe-path', 'unsupported writing-data target');
  }
  return target;
}

function ensureDir(dir) {
  if (transaction && dir !== transaction.root && !fs.existsSync(dir)) {
    contained(transaction.root, dir);
    ensureDir(path.dirname(dir));
    transaction.dirs.push(path.relative(transaction.root, dir));
    persistTransaction();
    fs.mkdirSync(dir);
    return;
  }
...
function readJson(file, fallback = null) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink > 1) throw new StyleError('unsafe-path', 'writing JSON must be a private regular file');
  try {
...
function rawWrite(file, value, encoding = 'utf8') {
  const suffix = `${transaction?.token || process.pid}-${++tempCounter}`;
  const temp = `${file}.tmp-${suffix}`;
  if (transaction && file !== path.join(transaction.root, '.pending.json')) {
    transaction.stages.push({ rel: path.relative(transaction.root, temp), hash: rawSha(value) });
    persistTransaction();
  }
  try {
...
function persistTransaction() {
  rawWrite(path.join(transaction.root, '.pending.json'), json(transaction));
}
...
function recordEffect(file, bytes) {
  if (!transaction) throw new StyleError('transaction-required', 'writing mutation requires an operation lock');
  contained(transaction.root, file);
  const rel = path.relative(transaction.root, file);
  let effect = transaction.effects.find(item => item.rel === rel);
  if (!effect) {
    effect = { rel, before: bytesAt(file)?.toString('base64') ?? null, after: [] };
    transaction.effects.push(effect);
  }
  const hash = bytes === null ? null : rawSha(bytes);
  if (!effect.after.includes(hash)) effect.after.push(hash);
  persistTransaction();
}
...
function atomicWrite(file, value, encoding = 'utf8') {
  ...
  recordEffect(file, bytes);
  rawWrite(file, bytes);
}

function removeFile(file) {
  recordEffect(file, null);
  fs.unlinkSync(file);
}

function readGuard(projectRoot) {
  const root = writingRoot(projectRoot);
  if (transaction?.root === root) return;
  for (const name of ['.pending.json', '.lock', '.recovery-lock']) {
    if (fs.existsSync(contained(root, path.join(root, name)))) throw new StyleError('operation-pending', 'writing data is locked or incomplete; inspect status before explicit recovery');
  }
}

function restoreOperation(record) {
  const root = record.root;
  // Validate the complete effect set before restoring anything. Retain intervening edits.
  const seen = new Set();
  for (const effect of record.effects) {
    if (typeof effect.rel !== 'string' || seen.has(effect.rel) || !/^(?:config\.json|styles[\\/][a-z0-9-]+[\\/].+)$/.test(effect.rel) || effect.rel.split(/[\\/]/).some(part => !part || part.startsWith('.')) || !Array.isArray(effect.after) || !effect.after.every(hash => hash === null || /^[a-f0-9]{64}$/.test(hash)) || (effect.before !== null && (typeof effect.before !== 'string' || Buffer.from(effect.before, 'base64').toString('base64') !== effect.before))) throw new StyleError('invalid-recovery', 'invalid recovery target');
    seen.add(effect.rel);
    const target = contained(root, path.join(root, effect.rel));
    const actual = bytesAt(target), before = effect.before === null ? null : Buffer.from(effect.before, 'base64');
    const hash = actual === null ? null : rawSha(actual);
    if (hash !== (before === null ? null : rawSha(before)) && !effect.after.includes(hash)) throw new StyleError('recovery-conflict', 'writing data changed after the retained operation; preserve it before recovery');
  }
  for (const rel of record.dirs) {
    if (typeof rel !== 'string' || !/^styles(?:[\\/][a-zA-Z0-9_-]+)*$/.test(rel)) throw new StyleError('invalid-recovery', 'invalid recovery directory');
    contained(root, path.join(root, rel));
  }
  for (const stage of record.stages) {
    if (typeof stage.rel !== 'string' || !stage.rel.includes(`.tmp-${record.token}-`)) throw new StyleError('invalid-recovery', 'invalid stage target');
    const bytes = bytesAt(contained(root, path.join(root, stage.rel)));
    if (bytes !== null && rawSha(bytes) !== stage.hash) throw new StyleError('recovery-conflict', 'a staged file changed; preserve it before recovery');
  }
  if (record.styleLock) {
    if (typeof record.styleLock.rel !== 'string' || !/^styles[\\/][a-z0-9-]+[\\/]\.lock$/.test(record.styleLock.rel) || typeof record.styleLock.ownership !== 'string') throw new StyleError('invalid-recovery', 'invalid style lock');
    const actual = bytesAt(contained(root, path.join(root, record.styleLock.rel)));
    if (actual !== null && actual.toString('utf8') !== record.styleLock.ownership) throw new StyleError('recovery-conflict', 'style lock ownership changed; preserve it before recovery');
  }
  for (const effect of [...record.effects].reverse()) {
    const target = contained(root, path.join(root, effect.rel));
    if (effect.before === null) { if (fs.existsSync(target)) fs.unlinkSync(target); }
    else rawWrite(target, Buffer.from(effect.before, 'base64'));
  }
  for (const stage of record.stages) {
    const file = contained(root, path.join(root, stage.rel)); if (fs.existsSync(file)) fs.unlinkSync(file);
  }
  if (record.styleLock) {
    const file = contained(root, path.join(root, record.styleLock.rel));
    if (bytesAt(file)?.toString('utf8') === record.styleLock.ownership) fs.unlinkSync(file);
  }
  for (const rel of [...record.dirs].reverse()) {
    const target = contained(root, path.join(root, rel));
    try { fs.rmdirSync(target); } catch (error) { if (!['ENOENT', 'ENOTEMPTY'].includes(error.code)) throw error; }
  }
}

function releaseProjectLock(root, ownership, token, disposition) {
  try {
    const lock = contained(root, path.join(root, '.lock')), current = bytesAt(lock);
    if (current === null) return;
    if (current.toString('utf8') !== ownership) throw new StyleError('ownership-changed', 'project lock ownership changed');
    fs.unlinkSync(lock);
  } catch (error) {
    throw Object.assign(new StyleError('lock-cleanup-required', `operation ${token}: data ${disposition}; preserve and reconcile .writing/.lock (${error.code || 'cleanup-failed'}). Do not repeat the mutation or attempt rollback of committed data.`), {
      operation: token, disposition, cleanup: '.writing/.lock', cause_code: error.code || 'cleanup-failed',
    });
  }
}

function mutate(projectRoot, command, requestedStyle, fn) {
  if (transaction) throw new StyleError('nested-operation', 'nested writing operations are unsupported');
  const root = writingRoot(projectRoot);
  readGuard(projectRoot);
  fs.mkdirSync(root, { recursive: true });
  // A local ignore protects data even in consumers whose root ignore predates this capability.
  const ignore = contained(root, path.join(root, '.gitignore'));
  if (!fs.existsSync(ignore)) fs.writeFileSync(ignore, '*\n', { flag: 'wx' });
  else if (!fs.readFileSync(ignore, 'utf8').split(/\r?\n/).includes('*')) throw new StyleError('privacy-ignore-required', 'protect .writing contents with a local * ignore before storing samples');
  const token = crypto.randomUUID(), lock = path.join(root, '.lock');
  const ownership = json({ token, pid: process.pid, command });
  try { fs.writeFileSync(lock, ownership, { flag: 'wx' }); }
  catch (error) { if (error.code === 'EEXIST') throw new StyleError('lock-held', 'another writing operation holds the project lock'); throw error; }
  let release = true, disposition = 'unchanged';
  try {
    // Recheck after acquiring ownership: recovery may have begun since readGuard.
    for (const name of ['.pending.json', '.recovery-lock']) {
      if (fs.existsSync(contained(root, path.join(root, name)))) throw new StyleError('operation-pending', 'retained operation or recovery guard requires reconciliation');
    }
    transaction = { schema: 1, root, token, command, effects: [], dirs: [], stages: [] };
    // Older managers use per-style locks. Hold that lock for the complete new operation too.
    const result = requestedStyle === null ? fn() : withLock(requireStyle(projectRoot, requestedStyle).dir, command, fn);
    const pending = path.join(root, '.pending.json');
    if (fs.existsSync(pending)) fs.unlinkSync(pending); // commit point: all coupled writes completed
    disposition = 'committed';
    return result;
  } catch (error) {
    if (!transaction) throw error;
    const record = transaction; transaction = null;
    try {
      restoreOperation(record);
      const pending = path.join(root, '.pending.json'); if (fs.existsSync(pending)) fs.unlinkSync(pending);
      disposition = 'restored-before-state';
    } catch (recoveryError) {
      release = false;
      throw new StyleError('recovery-required', `operation ${token} retained for recovery (${recoveryError.code || 'restore-failed'})`);
    }
    throw error;
  } finally {
    transaction = null;
    if (release) releaseProjectLock(root, ownership, token, disposition);
  }
}

export function operationStatus(projectRoot) {
  const root = writingRoot(projectRoot);
  const pending = readJson(contained(root, path.join(root, '.pending.json')));
  const lock = readJson(contained(root, path.join(root, '.lock')));
  const guard = readJson(contained(root, path.join(root, '.recovery-lock')));
  return { status: guard ? 'recovering' : pending ? 'recovery-required' : lock ? 'locked' : 'ready', operation: pending?.token || lock?.token || null, command: pending?.command || lock?.command || null };
}

export function recoverStyle(projectRoot, token, confirmed = false) {
  if (!confirmed || typeof token !== 'string' || !token) throw new StyleError('confirmation-required', 'verify the prior writer has stopped, then supply --operation <id> --confirm');
  const root = writingRoot(projectRoot), pendingPath = contained(root, path.join(root, '.pending.json')), lockPath = contained(root, path.join(root, '.lock'));
  // Exclusive recovery guard prevents two explicitly authorized recoveries from interleaving.
  return withLock(root, 'recovery-guard', () => {
    // Select the current generation only after acquisition; never restore a stale pre-lock snapshot.
    const record = readJson(pendingPath), lock = readJson(lockPath), ownership = bytesAt(lockPath)?.toString('utf8');
    if (!record || record.schema !== 1 || record.root !== root || record.token !== token || lock?.token !== token || !Array.isArray(record.effects) || !Array.isArray(record.dirs) || !Array.isArray(record.stages)) throw new StyleError('invalid-recovery', 'operation identity or recovery shape does not match');
    restoreOperation(record);
    fs.unlinkSync(pendingPath);
    releaseProjectLock(root, ownership, token, 'restored-before-state');
    return { status: 'recovered', operation: token, disposition: 'restored-before-state' };
  }, '.recovery-lock');
}
...
function configFile(projectRoot) {
  const root = writingRoot(projectRoot); return contained(root, path.join(root, 'config.json'));
}
...
export function loadWritingConfig(projectRoot) {
  ...
  if (!config.styles || typeof config.styles !== 'object' || Array.isArray(config.styles)) throw new StyleError('invalid-config', 'styles must be an object');
  const names = new Set();
  for (const [id, style] of Object.entries(config.styles)) {
    if (assertStyleId(id) !== id || !style || style.id !== id || !Array.isArray(style.aliases)) throw new StyleError('invalid-config', 'style registry identity is invalid');
    for (const name of [id, ...style.aliases]) {
      if (typeof name !== 'string' || !name.trim() || name !== name.trim().toLowerCase() || names.has(name)) throw new StyleError('alias-collision', 'style IDs and aliases must be unambiguous');
      names.add(name);
    }
  }
  if (config.default_style_id !== null && !Object.hasOwn(config.styles, config.default_style_id)) throw new StyleError('invalid-config', 'default style is not registered');
...
function sourceManifestFile(styleDir) {
  return contained(styleDir, path.join(styleDir, 'source-manifest.json'));
}
...
function styleMarkdownFile(styleDir) {
  return contained(styleDir, path.join(styleDir, 'style.md'));
}
...
function captureSource(options) {
  ...
  if (hasFile) {
    if (typeof options.file !== 'string' || !options.file.trim()) throw new StyleError('missing-content', '--file requires a path');
    const file = validateInputFile(options.file);
    const bytes = fs.readFileSync(file);
    const text = decodeUtf8(bytes);
    if (!text.trim()) throw new StyleError('missing-content', 'sample text is empty');
    return {
      bytes,
      text,
  ...
  if (typeof options.content !== 'string' || !options.content.trim()) throw new StyleError('missing-content', '--content requires nonempty text');
...
function buildArtifacts(style, styleDir, manifest, config) {
  ...
  const eligibleCandidates = chunks
  ...
  const approvedFile = contained(styleDir, path.join(styleDir, 'exemplars.approved.jsonl'));
  const approvedRows = fs.existsSync(approvedFile) ? fs.readFileSync(approvedFile, 'utf8').split('\n').filter(Boolean).map(line => JSON.parse(line)) : [];
  const eligibleRows = new Set(eligibleCandidates.map(candidate => JSON.stringify(candidate)));
  const approved = jsonLines(approvedRows.filter(row => eligibleRows.has(JSON.stringify(row))));
  ...
  return {
    artifacts: {
      ...
      'exemplar-candidates.jsonl': jsonLines(eligibleCandidates.slice(0, 50)),
...
function withLock(styleDir, command, fn, name = '.lock') {
  const lock = contained(styleDir, path.join(styleDir, name));
  const ownership = JSON.stringify({ token: crypto.randomUUID(), pid: process.pid, command });
  try {
    fs.writeFileSync(lock, ownership, { encoding: 'utf8', flag: 'wx' });
  } catch (error) {
  ...
  try {
    if (transaction && name === '.lock') {
      transaction.styleLock = { rel: path.relative(transaction.root, lock), ownership };
      persistTransaction();
    }
    return fn();
  } finally {
    if (bytesAt(lock)?.toString('utf8') === ownership) fs.unlinkSync(lock);
  }
}
...
function initStyleImpl(projectRoot, options = {}) {
  ...
  if (fs.existsSync(dir)) throw new StyleError('style-directory-exists', 'preserve and reconcile the existing unregistered style directory');
  ...
  for (const existing of Object.values(config.styles)) {
    if (existing.aliases.includes(id) || aliases.some((alias) => alias === existing.id || existing.aliases?.some((candidate) => String(candidate).toLowerCase() === alias))) {
  ...
  const built = buildAndPublish(root, resolved);
  recordRun(resolved, 'init', built);
  return { status: 'initialized', style_id: id, display_name: displayName, project_root: root, writing_root: writingRoot(root), profile_sha256: built.profileHash, contract_status: 'draft' };
}

function addSourceImpl(projectRoot, options = {}) {
  const root = normalizePath(projectRoot);
  ...
  atomicWrite(stored, capture.bytes);
...
function updateStyleImpl(projectRoot, requestedStyle) {
  const root = normalizePath(projectRoot);
...
export function inspectStyle(projectRoot, requestedStyle) {
  ...
  try { contract = styleContract(resolved.dir, resolved.style.id); } catch (error) { contractError = error.code; }
  const corpusDirty = !state || state.manifest_sha256 !== currentManifestHash;
  const integrity = auditStyle(root, requestedStyle).status === 'ok';
  return {
    ...
    corpus_dirty: corpusDirty,
    contract_valid: !!contract,
    contract_status: contract?.fm.contract_status || 'invalid',
    contract_error: contractError,
    ready_for_voice: !!contract && contract.fm.contract_status === 'reviewed' && !corpusDirty && integrity,
    dirty: corpusDirty || !contract || !integrity || state.style_md_sha256 !== contract.style_md_sha256,
    build_state: state,
    coverage: profile?.diagnostic_coverage || null,
  };
}

function excludeSourceImpl(projectRoot, sourceId, requestedStyle) {
  const root = normalizePath(projectRoot);
  const resolved = requireStyle(root, requestedStyle);
  const manifest = loadManifest(resolved.dir, resolved.style.id);
...
function purgeSourceImpl(projectRoot, sourceId, requestedStyle, confirmed = false) {
...
function approveExemplarImpl(projectRoot, candidateId, requestedStyle) {
  const root = normalizePath(projectRoot);
  const resolved = requireStyle(root, requestedStyle);
  const built = buildArtifacts(resolved.style, resolved.dir, loadManifest(resolved.dir, resolved.style.id), resolved.config);
  const candidate = built.artifacts['exemplar-candidates.jsonl'].split('\n').filter(Boolean).map(line => JSON.parse(line)).find(row => row.candidate_id === candidateId);
  if (!candidate) throw new StyleError('exemplar-not-found', `exemplar candidate not found: ${candidateId}`);
  const approvedFile = contained(resolved.dir, path.join(resolved.dir, 'exemplars.approved.jsonl'));
...
export function auditStyle(projectRoot, requestedStyle) {
  ...
  try {
    const built = buildArtifacts(resolved.style, resolved.dir, manifest, resolved.config);
    if (bytesAt(path.join(resolved.dir, 'exemplars.approved.jsonl'))?.toString('utf8') !== built.artifacts['exemplars.approved.jsonl']) findings.push({ code: 'stale-approved-exemplars', message: 'approved examples must match current included sources' });
  } catch (error) { findings.push({ code: error.code || 'invalid-artifact', message: error.message }); }
  const state = readJson(path.join(resolved.dir, 'build-state.json'), null);
  const profileBytes = bytesAt(contained(resolved.dir, path.join(resolved.dir, 'profile.generated.json')));
  if (!state || profileBytes === null || rawSha(profileBytes) !== state.generated_profile_sha256) findings.push({ code: 'profile-changed', message: 'generated profile does not match the completed build' });
...
export function initStyle(root, options = {}) {
  const id = assertStyleId(options.styleId || options.style || 'default');
  if (fs.existsSync(styleRoot(root, id))) throw new StyleError('style-directory-exists', 'preserve the existing style directory before initialization');
  return mutate(root, 'init', null, () => initStyleImpl(root, options));
}
export const addSource = (root, options = {}) => mutate(root, 'add', options.style, () => addSourceImpl(root, options));
export const updateStyle = (root, id) => mutate(root, 'update', id, () => updateStyleImpl(root, id));
export const excludeSource = (root, source, id) => mutate(root, 'exclude-source', id, () => excludeSourceImpl(root, source, id));
export const purgeSource = (root, source, id, confirmed = false) => mutate(root, 'purge-source', id, () => purgeSourceImpl(root, source, id, confirmed));
export const approveExemplar = (root, candidate, id) => mutate(root, 'approve-exemplar', id, () => approveExemplarImpl(root, candidate, id));

export function resolveStyleContext(root, id) {
  const resolved = requireStyle(root, id);
  const contract = styleContract(resolved.dir, resolved.style.id);
  if (contract.fm.contract_status !== 'reviewed') throw new StyleError('style-unreviewed', 'style.md is draft; its owner must review the voice guidance before use');
  const audit = auditStyle(root, id);
  if (audit.status !== 'ok') throw new StyleError('style-needs-attention', 'style source or exemplar integrity needs attention; run audit before using voice context');
  const built = buildArtifacts(resolved.style, resolved.dir, loadManifest(resolved.dir, resolved.style.id), resolved.config);
  readGuard(root);
  return { style_id: resolved.style.id, contract: contract.raw, approved_exemplars: built.artifacts['exemplars.approved.jsonl'].split('\n').filter(Boolean).map(line => JSON.parse(line)), diagnostics_status: built.diagnosticsStatus };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < rest.length; index += 1) {
    ...
    const rawKey = token.slice(2, equals < 0 ? undefined : equals), inline = equals < 0 ? undefined : token.slice(equals + 1);
    const key = rawKey.replace(/-([a-z])/g, (_, letter) => letter.toUpperCase());
    const allowed = new Set(['displayName', 'alias', 'default', 'file', 'content', 'style', 'authorship', 'rights', 'language', 'genre', 'mode', 'confirm', 'operation']);
    if (!allowed.has(key)) {
      throw new StyleError('unsupported-option', `--${rawKey} is not supported; writing styles are project-local`);
    }
    const boolean = ['default', 'confirm'].includes(key);
    const value = inline !== undefined ? inline : boolean ? true : (rest[index + 1] && !rest[index + 1].startsWith('--') ? rest[++index] : undefined);
    if (value === undefined || (!boolean && !String(value).trim())) throw new StyleError('missing-option-value', `--${rawKey} requires a value`);
    if (boolean && ![true, 'true', 'false'].includes(value)) throw new StyleError('invalid-option-value', `--${rawKey} requires true or false`);
...
function usage() {
  return [
    ...
    '  audit [style-id]',
    '  status',
    '  resolve [style-id] (read-only reviewed voice context)',
    '  recover --operation <id> --confirm (after verifying the prior writer has stopped)',
    '',
...
export function runCommand(argv, cwd = process.cwd()) {
  ...
  switch (command) {
    ...
    case 'status': return operationStatus(root);
    case 'resolve': return resolveStyleContext(root, styleArg(positionals, options));
    case 'recover': return recoverStyle(root, options.operation, options.confirm === true || options.confirm === 'true');
...
export function main(argv = process.argv.slice(2), cwd = process.cwd()) {
  try {
    ...
    for (const key of ['operation', 'disposition', 'cleanup', 'cause_code']) if (error[key] !== undefined) result[key] = error[key];
