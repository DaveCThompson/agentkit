...
function hooksFor(ctx, vendor) {
  const enabled = ctx.config?.commandGuard?.enabled === true;
  return (ctx.hooks || [])
    .filter(h => (!h.vendors || h.vendors.includes(vendor)) && (enabled || h.id !== 'command-guard'))
    .map(h => ({ ...h,
      ...(h.timeoutMs !== undefined ? { timeout: vendor === 'gemini' ? h.timeoutMs : h.timeoutMs / 1000 } : {}),
      ...(h.id === 'command-guard' && vendor === 'gemini' ? { event: 'BeforeTool', matcher: 'run_shell_command' } : {}),
      ...(h.command ? { command: h.command.replaceAll('{VENDOR}', vendor) } : {}),
      ...(h.commandWindows ? { commandWindows: h.commandWindows.replaceAll('{VENDOR}', vendor) } : {}),
    }));
}
...
function claude(entries, ctx) {
  ...
  const nativeHooks = hooksFor(ctx, 'claude');
...
function codex(entries, ctx) {
  ...
  const nativeHooks = hooksFor(ctx, 'codex');
  if (nativeHooks.length) settings.push({ file: '.codex/hooks.json', merge: 'codex-hooks', data: nativeHooks });
  return { files, settings, validations };
}
...
function gemini(entries, ctx) {
  ...
  const nativeHooks = hooksFor(ctx, 'gemini');
  if (nativeHooks.length) settings.push({ file: '.gemini/settings.json', merge: 'gemini-hooks', data: nativeHooks });
  return { files, settings, validations };
}
...
function opencode(entries, ctx) {
  ...
  if (ctx.config?.commandGuard?.enabled === true) {
    const source = entries.find(e => e.subPath === 'hooks.json')?.srcRel || '.agent/hooks.json';
    const content = "// Generated by AgentKit. Do not edit; run agentkit sync.\nimport { spawnSync } from 'node:child_process';\n\nexport default async function commandGuardPlugin({ directory, worktree }) {\n  return {\n    'tool.execute.before': async (input, output) => {\n      if (input?.tool !== 'bash' || typeof output?.args?.command !== 'string') return;\n      // Windows shell identity and direct .cmd binding are not proved. Activation is blocked.\n      if (process.platform === 'win32') return;\n      const launcher = 'agentkit';\n      const args = ['guard', '--vendor', 'opencode', '--json'];\n      let child;\n      try {\n        child = spawnSync(launcher, args, {\n          cwd: directory, input: JSON.stringify({\n            command: output.args.command, cwd: output.args.workdir || directory,\n            projectRoot: worktree || directory, tool_name: 'bash',\n            shell: 'posix',\n          }),\n          encoding: 'utf8', timeout: 1000, maxBuffer: 65536, windowsHide: true,\n          stdio: ['pipe', 'pipe', 'ignore'],\n        });\n      } catch { return; }\n      if (child.status !== 0 || !child.stdout) return;\n      let decision;\n      try { decision = JSON.parse(child.stdout); } catch { return; }\n      if (decision?.decision === 'deny') throw new Error(decision.reason || 'Destructive command denied by AgentKit.');\n    },\n  };\n}\n";
    files.push({ rel: '.opencode/plugins/agentkit-command-guard.js', content, source, transform: 'unsupported' });
  }
  return { files, settings, validations: [] };
}
