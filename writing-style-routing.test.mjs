...
const fixturePath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'writing-style-routing-fixture.json');
...
function countsByClass(cases) {
  return cases.reduce((groups, item) => { (groups[item.class] ||= []).push(item); return groups; }, {});
}
...
test('routing fixture hard-negative labels request no corpus mutation (not a model-routing test)', () => {
